...
class TiDBAdapter(SQLAdapter):
    Relation = TiDBRelation
    Column = TiDBColumn
    ConnectionManager = TiDBConnectionManager
    ...
    def list_relations_without_caching(
        self, schema_relation: TiDBRelation
    ) -> List[TiDBRelation]:
    ...
    def get_columns_in_relation(self, relation: Relation) -> List[TiDBColumn]:
        rows: List[agate.Row] = super().get_columns_in_relation(relation)
        return self.parse_show_columns(relation, rows)

    def _get_columns_for_catalog(
        self, relation: TiDBRelation
    ) -> Iterable[Dict[str, Any]]:
        columns = self.get_columns_in_relation(relation)

        for column in columns:
            # convert TiDBColumns into catalog dicts
    ...
    def get_rows_different_sql(
        self,
        relation_a: TiDBRelation,
        relation_b: TiDBRelation,
